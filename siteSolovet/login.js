...
async function login(username, password){
    try{
        const response = await fetch("ec2-3-131-137-213.us-east-2.compute.amazonaws.com:8080/usuarios/login", {
...
async function cadastro(username, password){
    try{
        const response = await fetch("ec2-3-131-137-213.us-east-2.compute.amazonaws.com:8080/usuarios/cadastro", {
        ...
        if(response.ok){
            const token = await fetch("ec2-3-131-137-213.us-east-2.compute.amazonaws.com:8080/usuarios/login", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
            ...
            const dataCadastro = await token.json();
            localStorage.setItem("tokenJWT", JSON.stringify(dataCadastro.token));
